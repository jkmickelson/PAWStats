...
PAWStats.showScreensAll = function () {
    myKeys = Object.keys(this.json);
    for (var i=0;i<myKeys.length;i++) {
        var site = myKeys[i];
        
        var t_chart = this.createScreensChart(site, true);       
        document.body.appendChild(t_chart);
    }
}

PAWStats.showScreensSection = function (site, showTitle) {
    showTitle = (showTitle!==false); // default value is true
    if (showTitle) {
        document.writeln('<h2 Section>Screen sizes<btn onclick="PAWStats.collapseSection(this)">–</btn></h2>');
    }
    
    var section = document.createElement('Section-Frame');
    var t_chart = this.createScreensChart(site);
    section.appendChild(t_chart);
    section.appendChild(document.createElement('br'));
    
    var t_data = this.createScreensData(site);
...
PAWStats.createScreensChart = function (site, showCaption) {
    ...
    var idsJson=sitedata.SCREENSIZE.ids;
    ...
    for (var itemArray in sortable) {
        ...
        var itemName = itemID.replace(/([0-9])(x)([0-9])/, "$1$2<br>$3");
...
PAWStats.createScreensData = function (site) {
    var sitedata=this.json[site][this.year+''+this.month];
