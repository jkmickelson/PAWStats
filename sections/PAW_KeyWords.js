...
/*
PAWStats.showKeywordsAll = function () {
    myKeys = Object.keys(this.json);
    for (var i=0;i<myKeys.length;i++) {
        var site = myKeys[i];
        
        var t_chart = this.createKeywordsChart(site, true);       
        document.body.appendChild(t_chart);
    }
}
*/
PAWStats.showKeywordsSection = function (site, showTitle) {
    showTitle = (showTitle!==false); // default value is true
    if (showTitle) {
        document.writeln('<h2 Section>Search Keywords<btn onclick="PAWStats.collapseSection(this)">–</btn></h2>');
    }
    
    var section = document.createElement('Section-Frame');   
    var t_data = this.createKeywordsData(site);
...
PAWStats.createKeywordsData = function (site) {
    ...
    var idsJson=sitedata.KEYWORDS.ids;
    ...
    for (var dataID in idsJson) {
        ...
        FETCH = KEYWORDS.count * 1;
        TD.innerHTML = FETCH._toLocalNo();
        this.SumHits+=FETCH;
        
        TD = TR.insertCell(); TD.className = 'hdr';
        FETCH = KEYWORDS.count * 1 / SUM.count *100;
        FETCH = FETCH || 0;
